...
def main():
    ...
    parser.add_argument('--model', default='maxmil', type=str, help='MIL model [dsmil]')
    ...
    milnet = maxmil(input_size=args.feats_size).cuda()
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epochs, 0.000001)
    ...
    best_score = 10**(10)
    save_path = os.path.join('weights/maxmil', datetime.datetime.now().strftime("%Y%m%d%H%M%S%f"))
    print(save_path)
