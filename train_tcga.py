...
def best_test(test_df, milnet, args):
    milnet.eval()
...
def main():
    ...
    b_classifier = mil.BClassifier(input_size=args.feats_size, output_class=args.num_classes, dropout_v=args.dropout_node, nonlinear=args.non_linearity,q_n=args.q_n).cuda()
    milnet = mil.MILNet(i_classifier, b_classifier).cuda()
    if args.model == 'dsmil':
        state_dict_weights = torch.load('init/'+str(args.feats_size)+'/'+str(args.q_n)+'.pth')
        try:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epochs, 0.0000001)
    ...
    # val_path = bags_path.iloc[int(len(bags_path)*(1-args.split)):, :]
    # weights/20230403192123745396/1.pth
    best_score = 10**(10)
    save_path = os.path.join('weights', datetime.datetime.now().strftime("%Y%m%d%H%M%S%f"))
    print(save_path)
